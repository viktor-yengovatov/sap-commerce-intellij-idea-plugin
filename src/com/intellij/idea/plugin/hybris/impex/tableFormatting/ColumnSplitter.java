...
import static com.intellij.idea.plugin.hybris.impex.tableFormatting.util.ImpexTableFormatterConstants.PIPE_COMMENT_END;
...
public class ColumnSplitter implements Iterable<String>, Iterator<String> {
    ...
    private void init() {
        ...
        if (line.startsWith(PIPE_COMMENT_START)) {
            ...
            commented = true;
            leadingPipe = true;
        } else if (line.startsWith(Character.toString(PIPE))) {
            startIndex = 1;
            leadingPipe = true;
        }
        if (line.endsWith(PIPE_COMMENT_END)) {
            ...
            trailingPipe = true;
        } else if (line.endsWith(Character.toString(PIPE))) {
            length--;
            trailingPipe = true;
        }
        columnIndex = -1;
    ...
    private boolean toDelimiter(int index) {
        return !insideQuoted && line.charAt(index) == delimiter;
    }
