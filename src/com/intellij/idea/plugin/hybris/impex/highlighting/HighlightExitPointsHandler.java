...
public class HighlightExitPointsHandler extends HighlightUsagesHandlerBase<PsiElement> {
    ...
    public void computeUsages(final List<PsiElement> targets) {
        System.out.println("HighlightExitPointsHandler.computeUsages");
        final PsiFile file = myTarget.getContainingFile();

        final PsiElement[] psiElements = PsiTreeUtil.collectElements(
            file,
            element -> CommonPsiUtils.getNullSafeElementType(element) == ImpexTypes.MACRO_NAME_DECLARATION
                       || CommonPsiUtils.getNullSafeElementType(element) == ImpexTypes.MACRO_USAGE
        );
