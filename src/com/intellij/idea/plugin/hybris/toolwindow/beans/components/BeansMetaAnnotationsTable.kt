...
class BeansMetaAnnotationsTable private constructor(myProject: Project) :
    AbstractTable<BeansGlobalMetaBean, BeansMetaAnnotations>(myProject) {
    ...
    override fun select(meta: BeansMetaAnnotations) = selectRowWithValue(meta.value, COLUMN_VALUE)
    override fun getItems(meta: BeansGlobalMetaBean) = meta.annotations.sortedWith(
        compareBy(
            { !it.isCustom },
            { it.module.name },
            { it.value })
    ...
    override fun createModel(): ListTableModel<BeansMetaAnnotations> = with(ListTableModel<BeansMetaAnnotations>()) {
        columnInfos = arrayOf(
            createColumn(
                name = COLUMN_CUSTOM,
    ...
    companion object {
        ...
        fun getInstance(project: Project): BeansMetaAnnotationsTable = with(BeansMetaAnnotationsTable(project)) {
            init()
