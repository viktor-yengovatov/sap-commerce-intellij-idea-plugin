...
class BSTreeModel(private val root: BSNode)
    : BaseTreeModel<BSTreeModel.Node>(), Disposable, InvokerSupplier {
    ...
    override fun getChildren(parent: Any?): List<Node> {
        if (parent !is Node || !parent.allowsChildren || parent.userObject !is BSNode || DumbService.isDumb(root.project)) {
            return emptyList();
        }

        return (parent.userObject as BSNode).getChildren()
            .onEach { it!!.update() }
            .map { Node(it) }
    }
    ...
    class Node(private val node : BSNode?) : DefaultMutableTreeNode(node) {
