...
class TSQualifierMustExistForNavigablePartInN2MRelation : AbstractTSInspection() {
    override fun inspect(
        ...
        dom.relations.relations
            ...
            .flatMap { listOf(it.sourceElement, it.targetElement) }
            .filter { it.navigable.value && !it.qualifier.exists() }
            .forEach {
                holder.createProblem(
                    it,
                    severity,
                    displayName,
                    XmlAddAttributeQuickFix(RelationElement.QUALIFIER)
                )
            }
        }
}